...
double simpson(int n, double left_limit, double right_limit, bool table);

int main()
{
    double wolfram = 275.5109837633;                 // Resultado Wolfram|Alpha
    double left_limit = 1.0;                         // Límite izquierdo
    double right_limit = 2.0;                        // Límite derecho
    ...
    std::cout << "\nMenor valor de n: " << n << "\n";
    std::cout << "Aproximación integral (con regla de Simpson):\n";
    std::cout << "\\int_0^1(e^{x^{3}}) = " << std::setprecision(10) << std::fixed << result << "\n\n";
...
int findn(double solution, double left_limit, double right_limit)
{
    ...
    int n = 2;
    for (n; comparison > 1e-4; n += 2)      // Revisa que | S - W | < 10^{-4}
    {
        comparison = std::abs(solution - simpson(n, left_limit, right_limit, false)); // Valor absoluto
        if (comparison <= 1e-4)
        {
            break;
        }
...
double simpson(int n, double left_limit, double right_limit, bool table)
{
    ...
    double result = (delta_x / 3) * (sum - fx(left_limit) - fx(right_limit));
